...
- `grep`, `ast_grep`, and `ast_edit` take a scope root and then recurse beneath it. The file set they report visiting is re-evaluated after the call and before the result reaches the model, so `grep --path .` cannot surface a denied file's contents. A denial also discards any preview the call staged, so the pending `xd://resolve` cannot apply it afterwards.

  **Known gap.** That recheck only sees files that *matched*. The native searcher reports how many files it searched but not which, and takes no exclusion globs, so a denied file beneath an allowed root is still opened when it matches nothing — and because a denial fires only on a match, a caller can use it as a "does this file contain X" oracle. Closing it properly needs the native searcher to accept exclusions or return its visited set. Until then, deny the containing directory rather than the file if you need a recursive search to stay away from it.
