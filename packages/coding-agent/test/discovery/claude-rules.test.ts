...
describe("Claude Code rule discovery", () => {
	...
	test("respects gitignore for rules under symlinked directories", async () => {
		const sharedRulesDir = path.join(root, "shared-claude-rules");
		await writeFile(path.join(project, ".gitignore"), ".claude/rules/shared/ignored.md\n");
		await writeFile(path.join(sharedRulesDir, "ignored.md"), "Ignored shared rule.\n");
	...
	test("filters native matches when the rules root itself is symlinked", async () => {
		const sharedRulesDir = path.join(root, "shared-claude-rules");
		...
		await fs.rm(path.join(project, ".claude", "rules"), { recursive: true, force: true });
		await fs.symlink(sharedRulesDir, path.join(project, ".claude", "rules"), "dir");

		const result = await loadCapability<Rule>(ruleCapability.id, { cwd: project, providers: ["claude"] });

		expect(result.items.map(rule => rule.name)).toEqual(["kept"]);
		expect(result.items[0]?.content).toBe("Kept shared rule.\n");
	});

	test("respects gitignore directory patterns for symlinked rule directories", async () => {
		const sharedRulesDir = path.join(root, "shared-claude-rules");
		await writeFile(path.join(project, ".gitignore"), ".claude/rules/shared\n");
		await writeFile(path.join(sharedRulesDir, "kept.md"), "Ignored shared rule.\n");
		await fs.mkdir(path.join(project, ".claude", "rules"), { recursive: true });
		await fs.symlink(sharedRulesDir, path.join(project, ".claude", "rules", "shared"), "dir");
	...
	test("respects gitfile info excludes for symlinked rule files", async () => {
		const sharedRulesDir = path.join(root, "shared-claude-rules");
		const gitDir = path.join(root, "gitdir-store");
		await fs.rm(path.join(project, ".git"), { recursive: true, force: true });
		await writeFile(path.join(project, ".git"), `gitdir: ${gitDir}\n`);
		await writeFile(path.join(gitDir, "info", "exclude"), ".claude/rules/shared/private.md\n");
		await writeFile(path.join(sharedRulesDir, "private.md"), "Private shared rule.\n");
		await writeFile(path.join(sharedRulesDir, "kept.md"), "Kept shared rule.\n");
		await fs.mkdir(path.join(project, ".claude", "rules"), { recursive: true });
		await fs.symlink(sharedRulesDir, path.join(project, ".claude", "rules", "shared"), "dir");
	...
	test("respects configured global excludes for symlinked rule files", async () => {
		const sharedRulesDir = path.join(root, "shared-claude-rules");
		await writeFile(path.join(home, ".gitconfig"), "[core]\n\texcludesFile = ~/.gitignore_global\n");
		await writeFile(path.join(home, ".gitignore_global"), ".claude/rules/shared/private.md\n");
		await writeFile(path.join(sharedRulesDir, "private.md"), "Private shared rule.\n");
		await writeFile(path.join(sharedRulesDir, "kept.md"), "Kept shared rule.\n");
		await fs.mkdir(path.join(project, ".claude", "rules"), { recursive: true });
		await fs.symlink(sharedRulesDir, path.join(project, ".claude", "rules", "shared"), "dir");
	...
	test("does not reinclude files under wildcard-ignored symlinked rule directories", async () => {
		const sharedRulesDir = path.join(root, "shared-claude-rules");
		await writeFile(path.join(project, ".gitignore"), "*\n!.claude/rules/shared/kept.md\n");
		await writeFile(path.join(sharedRulesDir, "kept.md"), "Still ignored by wildcard parent.\n");
		await fs.mkdir(path.join(project, ".claude", "rules"), { recursive: true });
		await fs.symlink(sharedRulesDir, path.join(project, ".claude", "rules", "shared"), "dir");
