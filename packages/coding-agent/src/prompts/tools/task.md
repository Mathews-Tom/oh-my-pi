...
# Task Design
- **Agent typing:** Pick each item's `agent` type. Read-only research MUST use `agent: "scout"` (faster model). Omit `agent` to use `{{defaultAgent}}`.{{#if allowedPromptText}} Allowed: {{allowedPromptText}}.{{/if}}
